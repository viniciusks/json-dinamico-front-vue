...
-->
			...
			<!-- CAMPO PAR - LISTA DE DADOS -->
			<div class="row" v-for="item in campoPar">
				<div v-if="item.tipo == '1'">
					<select class="form-control widthDefault inlineBlock" :value="item.tipo">
						<option value="1">Par</option>
						<option value="2">Array</option>
						<option value="3">Object</option>
					</select>
					<input class="form-control widthDefault inlineBlock" placeholder="Key..." :value="item.key">
					<input class="form-control widthDefault inlineBlock" placeholder="Value..." :value="item.value">	
				</div>
				<div v-if="item.tipo == '2'">
					<div v-for="val in item.value">
						...
						<input class="form-control widthDefault inlineBlock" placeholder="Key..." :value="item.key">
						<input class="form-control widthDefault inlineBlock" placeholder="Value..." :value="val">
					</div>
				</div>
			</div>

			<!-- CAMPO ARRAY - LISTA DE DADOS -->
			<div class="row" v-for="item in campoArray">
				<div v-if="item.tipo == '1'">
					<select class="form-control widthDefault inlineBlock" :value="item.tipo">
						<option value="1">Par</option>
						<option value="2">Array</option>
			...
			<!-- CAMPO OBJECT - LISTA DE DADOS -->
			<div class="row" v-for="item in campoObject">
				<div v-if="item.tipo == '1'">
					<select class="form-control widthDefault inlineBlock" :value="item.tipo">
						<option value="1">Par</option>
						<option value="2">Array</option>
					...
					<span class="form-control widthDefault3 inlineBlock" disabled>{{ item.nameObj }}</span>
					<input class="form-control widthDefault inlineBlock" placeholder="Key..." :value="item.key">
					<input class="form-control widthDefault inlineBlock" placeholder="Value..." :value="item.value">	
				</div>
				<div v-if="item.tipo == '2'">
			...
			<!-- CAMPOS - CRIAÇÃO DE DADOS -->
			<div class="row" v-for="json in campos">
				<select class="form-control widthDefault inlineBlock" v-on:click="zeraDado(json)" v-model="json.tipo">
					<option value="1">Par</option>
					<option value="2">Array</option>
					<option value="3">Object</option>
				</select>
				<div class="inlineBlock" v-if="json.tipo == '1' || json.tipo == '2'">
					<input id="inputKey" class="form-control widthDefault inlineBlock" placeholder="Key..." v-model="json.key">
					<input id="inputValue" class="form-control widthDefault inlineBlock" placeholder="Value..." v-model="json.value">
					<button class="marg5 btn btn-primary inlineBlock" v-on:click="addLinha(json.tipo,json.key,json.value),zeraDado(json)">Adicionar</button>
				</div>
				<div class="inlineBlock" v-if="json.tipo == '3'">
					...
					<button class="marg5 btn btn-primary inlineBlock" v-on:click="addObject(json.nameObj,'1')">Adicionar Objeto</button>
				</div>
				<br>
				<div v-if="json.tipo == '3'" v-for="campo2 in campos2">
					...
					<div class="inlineBlock" v-if="campo2.tipo == '1' || campo2.tipo == '2'">
						<input id="inputKey" placeholder="Key..." v-model="campo2.key" class="form-control widthDefault inlineBlock">
						<input id="inputValue" placeholder="Value..." v-model="campo2.value" class="form-control widthDefault inlineBlock">
						<button class="marg5 btn btn-primary inlineBlock" v-on:click="addLinhaObject(campo2.tipo,campo2.nameObj,campo2.key,campo2.value),zeraDado(campo2)">Adicionar</button>
						<button class="marg5 btn btn-danger inlineBlock" v-on:click="sairObject(campos2)">Sair</button>
					</div>
...
export default {
  ...
  data: function(){
    return {
			...
			campoPar: this.$store.state.jsonPar,
			campoArray: this.$store.state.jsonArray,
